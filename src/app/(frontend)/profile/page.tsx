...
import { User, Package, MapPin, CreditCard, Bell, LogOut, ChevronRight, Edit2, Calendar } from "lucide-react"
...
export default function ProfilePage() {
  ...
  // Завантажуємо дані користувача та інші дані
  useEffect(() => {
    const token = localStorage.getItem("token")
    if (!token) {
      window.location.href = "/"; // Перенаправлення, якщо користувач не авторизований
      return;
    }
    ...
    axios
      .get("/api/profile/orders", { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        toast.error("Не вдалося завантажити адреси доставки.")
      })

    // Завантажуємо способи оплати
    axios
      .get("/api/profile/payment-methods", { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
        setPaymentMethods(response.data.paymentMethods)
      })
      .catch((error) => {
        console.error("Error loading payment methods:", error)
        toast.error("Не вдалося завантажити способи оплати.")
      })

    // Завантажуємо налаштування сповіщень
    axios
      .get("/api/profile/notifications", { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
    ...
    try {
      ...
      await axios.put(
        "/api/profile/notifications",
        {
          ...
          promoNotifications: notificationSettings.promoNotifications,
          newsNotifications: notificationSettings.newsNotifications,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      ...
      toast.success(err.response?.data?.error || "Щось пішло не так. Спробуйте ще раз.")
    } finally {
      setLoading(false)
    }
  }
