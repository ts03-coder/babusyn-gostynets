...
export default function CartPage() {
  ...
  const fetchCart = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Не вдалося завантажити кошик. Спробуйте пізніше.")
      }
    } catch (error) {
      console.error('Помилка завантаження кошика:', error)
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.message === "Товар видалено з корзини") {
          // Якщо кількість стала < 1, API видалив товар
          setCartItems(prev => prev.filter(item => item.productId !== productId))
          toast.success("Товар видалено з корзини.")
        } else {
          // Оновлюємо кількість у локальному стані
          setCartItems(prev => 
            prev.map(item => 
        ...
        // Повторно завантажуємо кошик для синхронізації
        fetchCart()
      }
    } catch (error) {
      console.error('Помилка оновлення кількості:', error)
  ...
  // Завантаження даних кошика при монтуванні компонента
  useEffect(() => {
    fetchCart()
  }, [])
  ...
  const discount = 200 // Приклад знижки (в майбутньому можна отримувати з API)
  const delivery = 100 // Приклад вартості доставки (в майбутньому можна отримувати з API)
  const total = subtotal - discount + delivery

  // Перевірка, чи корзина порожня
  const isCartEmpty = cartItems.length === 0

  // Відображення спінера завантаження
  if (loading) {
    return (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    {cartItems.map((item) => (
                      <div key={item.id} className="bg-white rounded-lg p-4 shadow-sm">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                          ...
                          {/* Ціна (тільки десктоп) */}
                          <div className="hidden md:block md:col-span-2 text-center font-medium">{item.price} ₴</div>
              ...
              <div className="lg:w-1/3">
                <div className="bg-gray-50 rounded-lg p-6 sticky top-4">
                  ...
                  <Link href="/checkout">
                    <Button className="w-full py-6 text-base rounded-lg" disabled={processing}>
                      {processing ? (
